...
import {
  useAddMenuItemComponent,
  useRemoveMenuItemComponent,
  useUpdateMenuItemComponent,
...
import type { MenuItemWithDetails, Recipe } from '@/types'

const NO_UNIT = '__none__'

export function MenuItemComponentsDialog({
  open,
  onOpenChange,
  menuId,
  menuItem,
  ...
  const addComponent = useAddMenuItemComponent(menuId)
  const updateComponent = useUpdateMenuItemComponent(menuId)
  const removeComponent = useRemoveMenuItemComponent(menuId)
    ...
    try {
      if (kind === 'recipe') {
        ...
        await addComponent.mutateAsync({
          menu_item_id: menuItem!.id, recipe_id: recipe.id, quantity,
          unit_id: unitId === NO_UNIT ? null : unitId, sort_order: components.length,
        })
      } else {
        if (!ingredientId) { toast.error('Zutat wählen'); return }
        await addComponent.mutateAsync({
          menu_item_id: menuItem!.id, ingredient_id: ingredientId, quantity,
          unit_id: unitId === NO_UNIT ? null : unitId, sort_order: components.length,
        })
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Komponenten — {menuItem.name}</DialogTitle>
          <DialogDescription>
            Bestandteile dieser Position: zugekaufte/rohe Zutaten + vorproduzierte Rezepte, je Menge pro Portion.
            Sind Komponenten gesetzt, ersetzen sie die einzelne Rezept-Verknüpfung in der Bedarfsrechnung.
          </DialogDescription>
        </DialogHeader>
