-- ============================================================
-- OSD Catering - PDF catalog corrections for import/add-ons/recipes
-- Migration: 20260626000003_pdf_catalog_addons_recipe_stubs
--
-- Uses OSD-Speisen-komplett.pdf as the fachliche reference.
-- - Removes persisted "variant" import fields in favor of expected_item_count.
-- - Marks menu add-ons explicitly.
-- - Adds recipe review flags and incomplete recipe stubs for PDF positions
--   that are production/calculation-relevant but lack components.
-- - Adds obvious multi-component links where the PDF names the components.
-- ============================================================

-- Import review schema: keep the useful position-count hint from "3/6/9 Teile",
-- but remove the fachlich wrong free-text variant fields.
DO $$
BEGIN
  IF EXISTS (
...
-- Explicit add-on flags. menu_positions.is_add_on is the menu-specific truth;
-- positions.is_add_on is a convenience for search/import when a shared position
...
-- Recipe review flags. Existing complete recipes stay active; generated stubs
-- are explicitly incomplete and review-worthy.
...
-- Caesar Salad is fachlich split into with/without Hähnchen so imports can map
-- the sold variant through the selected position, not through a separate import field.
