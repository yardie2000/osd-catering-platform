...
async function checkConnection(): Promise<{ status: ConnectionStatus; detail: string }> {
  ...
  try {
    ...
    return { status: 'ok', detail: 'Verbunden - Schema V5.3.0 aktiv' }
  } catch (err) {
...
export default async function SettingsPage() {
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader title="Einstellungen" description="Plattformkonfiguration, Schema-Status und V5.3.0-Betrieb" />
      <div className="max-w-3xl space-y-6 p-4 sm:p-6 lg:p-8">
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="success">V5.3.0</Badge>
            </div>
          </CardHeader>
          <CardContent className="text-sm space-y-3 text-muted-foreground">
            <p className="text-xs">
              Stammdaten, Rezepte, Menues, Positionen und Add-ons sind aktiv. In V5.3.0 ist der MouseClick-Produktbedarfimport als Review-Workflow verfuegbar: Events, verkaufte Menues, erwartete Positionsanzahl und tatsaechlich gewaehlte Kundenpositionen werden getrennt gespeichert.
            </p>
        ...
        {/* V5.3.0 Operations workflow */}
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">Betriebsablauf (V5.3.0)</CardTitle>
